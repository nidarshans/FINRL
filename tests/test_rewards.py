...
def test_reward_positive_when_net_return_is_positive() -> None:
...
def test_reward_ignores_spy_return_when_net_return_is_positive() -> None:
    reward = calculate_rewards(
    ...
    expected = np.log1p(0.005)

    assert_allclose(reward, expected, rtol=RTOL, atol=ATOL)


def test_reward_handles_up_period() -> None:
    reward = calculate_rewards(
    ...
    expected = np.log1p(0.03)

    assert_allclose(reward, expected, rtol=RTOL, atol=ATOL)


def test_reward_handles_down_period() -> None:
    reward = calculate_rewards(
    ...
    expected = np.log1p(-0.01) - RewardConfig().sortino_downside_penalty * 0.01**2
...
def test_turnover_penalty_is_currently_ignored() -> None:
    ...
    high_turnover_reward = calculate_rewards(
        net_return=jnp.array(0.0, dtype=jnp.float32),
        spy_return=jnp.array(0.0, dtype=jnp.float32),
        drawdown=jnp.array(0.0, dtype=jnp.float32),
